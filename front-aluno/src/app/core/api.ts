...
export const urlsApi = {
  academico: semBarraFinal(environment.apiUrl || "http://localhost:8080/api"),
  materiais: semBarraFinal(environment.materiaisApiUrl || "http://localhost:8000/api"),
};
...
export function urlDaApi(caminho: string, urlBase: string = urlsApi.academico): string {
  if (/^https?:\/\//i.test(caminho)) return caminho;
  return `${semBarraFinal(urlBase)}${caminho.startsWith("/") ? "" : "/"}${caminho}`;
}

async function requisicao(
  caminho: string,
  signal?: AbortSignal,
  urlBase: string = urlsApi.academico,
): Promise<unknown> {
  const controladorTempo = new AbortController();
  const expirou = setTimeout(() => controladorTempo.abort(new Error("timeout")), TEMPO_LIMITE);

  try {
    const resposta = await fetch(urlDaApi(caminho, urlBase), {
...
export async function buscarLista<T>(
  caminho: string,
  { signal, ignorarCache = false, urlBase = urlsApi.academico }: OpcoesDeBusca = {},
): Promise<T[]> {
  const agora = Date.now();
  const chaveDoCache = urlDaApi(caminho, urlBase);
  const emCache = cache.get(chaveDoCache);

  if (!ignorarCache && emCache && agora - emCache.em < VALIDADE_CACHE) {
    return emCache.dados as T[];
  }

  const dados = comoLista(await requisicao(caminho, signal, urlBase));
  cache.set(chaveDoCache, { dados, em: agora });
  return dados as T[];
}
...
export function urlAbsoluta(
  ...
  try {
    return new URL(url, `${semBarraFinal(urlBase)}/`).toString();
  } catch {
    return null;
  }
}
...
export const rotas = {
  ...
  materiais: (disciplinaId?: string | number): string =>
    disciplinaId ? `/materiais?disciplina=${encodeURIComponent(disciplinaId)}` : "/materiais",
  baixarMaterial: (id: string | number): string => `/materiais/${encodeURIComponent(id)}`,
};
