...
describe("normalizarAnexos", () => {
  ...
  it("aceita a lista como strings puras", () => {
    const anexos = normalizarAnexos({ anexos: ["/arquivos/lista de presença.pdf"] });
  ...
  it("inventa um nome quando a API não manda", () => {
    const anexos = normalizarAnexos({ arquivoUrl: "https://api/files/2026/edital.pdf" });
...
describe("formatarTamanho", () => {
  ...
  it("devolve null quando não há tamanho", () => {
    ...
    expect(formatarTamanho("muito grande")).toBeNull();
  });
});

describe("extensao", () => {
  it("tira do nome do arquivo", () => {
    expect(extensao({ nome: "prova.pdf" })).toBe("PDF");
  });

  it("cai para o mime type quando o nome não tem extensão", () => {
    expect(extensao({ nome: "arquivo", tipo: "image/png" })).toBe("PNG");
  });

  it("tem um rótulo de reserva", () => {
    expect(extensao({ nome: "arquivo" })).toBe("ARQUIVO");
  });
});
