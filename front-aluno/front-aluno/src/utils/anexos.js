...
function normalizarUm(bruto, indice) {
  if (!bruto) return null;

  // A API pode mandar só a string da URL.
  const objeto = typeof bruto === "string" ? { url: bruto } : bruto;
  ...
  return {
    id: objeto.id ?? `anexo-${indice}`,
    url,
    nome: String(primeiroValor(objeto, CHAVES_NOME) || nomePelaUrl(url)),
    tipo: primeiroValor(objeto, CHAVES_TIPO),
    tamanho: Number.isFinite(tamanho) && tamanho > 0 ? tamanho : null,
  };
}

export function normalizarAnexos(aviso) {
  if (!aviso) return [];

  if (Array.isArray(aviso.anexos)) {
    return aviso.anexos.map(normalizarUm).filter(Boolean);
  }
  ...
  if (aviso.arquivoUrl || aviso.url) {
    const unico = normalizarUm(aviso, 0);
    return unico ? [unico] : [];
  }
...
export function extensao(anexo) {
