...
describe("App", () => {
  ...
  it("mostra o botão de download quando o aviso tem arquivo", async () => {
    ...
    const link = await screen.findByRole("link", { name: /baixar roteiro-prova\.pdf/i });
    expect(link).toHaveAttribute("download", "roteiro-prova.pdf");
    expect(link).toHaveAttribute("href", "http://localhost:8080/arquivos/roteiro-prova.pdf");
  ...
  it("não mostra nada de download no aviso sem arquivo", async () => {
    ...
    expect(screen.getAllByRole("link", { name: /baixar/i })).toHaveLength(1);
  });
});
